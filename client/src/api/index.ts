 ...
 * 获取势力领地列表（用于商业投资）
 */
export async function getFactionTerritoriesForCommerce(factionId?: string): Promise<ApiResponse<Territory[]>> {
  const endpoint = factionId ? `/investment/commerce/territories?factionId=${factionId}` : '/investment/commerce/territories';
  return request<Territory[]>(endpoint);
}
 ...
 * 获取商业投资预览
 */
export async function getCommerceInvestmentPreview(
  samuraiId: string,
  territoryId: string,
  amount: number,
  factionId?: string
): Promise<ApiResponse<CommerceInvestmentPreview>> {
  return request<CommerceInvestmentPreview>('/investment/commerce/preview', {
    method: 'POST',
    body: JSON.stringify({ samuraiId, territoryId, amount, factionId }),
  });
}
 ...
 * 执行商业投资
 */
export async function executeCommerceInvestment(
  samuraiId: string,
  territoryId: string,
  amount: number,
  factionId?: string
): Promise<ApiResponse<CommerceInvestmentResult>> {
  return request<CommerceInvestmentResult>('/investment/commerce/execute', {
    method: 'POST',
    body: JSON.stringify({ samuraiId, territoryId, amount, factionId }),
  });
}
...
// ============ 重置系统 API (Requirements: 6.1-6.5, 2.5.1-2.5.6) ============
 ...
 * 获取重置系统状态（管理员）
 */
export async function getResetStatus(): Promise<ApiResponse<ResetStatus>> {
  return request<ResetStatus>('/game/reset/status');
}
 ...
 */
export async function executeReset(confirmText: string): Promise<ApiResponse<void>> {
  return request<void>('/game/reset', {
    method: 'POST',
 ...
 * Requirements: 2.5.2
 */
export async function getInitialData(): Promise<ApiResponse<InitialData>> {
  return request<InitialData>('/game/initial-data');
}
 ...
 * Requirements: 2.5.1
 */
export async function saveCurrentAsInitialData(): Promise<ApiResponse<void>> {
  return request<void>('/game/initial-data/save', {
    method: 'POST',
  });
}
 ...
 * Requirements: 2.5.5
 */
export async function updateInitialData(territories: Territory[]): Promise<ApiResponse<void>> {
  return request<void>('/game/initial-data', {
    method: 'PUT',
 ...
 */
export async function updateAdminCode(newCode: string): Promise<ApiResponse<void>> {
  return request<void>('/game/admin-code', {
    method: 'PUT',
 ...
 * Requirements: 8.6
 */
export async function getSecurityKey(): Promise<ApiResponse<{ securityKey: string }>> {
  return request<{ securityKey: string }>('/game/security-key');
}
