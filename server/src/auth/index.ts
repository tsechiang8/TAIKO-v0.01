...
/**
 * 验证登录代码并创建会话
 * Requirements: 8.4 - 支持管理员代码或安全密钥登录
 * @param code 登录代码
 * @returns 认证结果
 */
export function login(code: string): AuthResult & { sessionId?: string } {
  ...
  // 检查是否为管理员代码或安全密钥
  const isAdminCode = trimmedCode === gameState.adminCode;
  const isSecurityKey = gameState.securityKey && trimmedCode === gameState.securityKey;
  
  if (isAdminCode || isSecurityKey) {
    console.log('Admin login successful via', isAdminCode ? 'admin code' : 'security key');
