...
/**
 * GET /api/investment/commerce/territories
 * 获取势力领地列表（用于商业投资）
 */
router.get('/commerce/territories', requireAuth, (req: Request, res: Response) => {
  if (!req.sessionId) {
    res.status(401).json({
      success: false,
      error: '未认证',
  ...
  const territories = getFactionTerritories(factionId);
...
/**
 * POST /api/investment/commerce/preview
 * 获取商业投资预览
 */
router.post('/commerce/preview', requireAuth, (req: Request, res: Response) => {
  if (!req.sessionId) {
    res.status(401).json({
      success: false,
      error: '未认证',
  ...
  const { samuraiId, territoryId, amount, factionId: requestFactionId } = req.body;
  ...
  const preview = getCommerceInvestmentPreview(
    factionId,
    samuraiId,
    territoryId,
    Number(amount)
...
/**
 * POST /api/investment/commerce/execute
 * 执行商业投资
 */
router.post('/commerce/execute', requireAuth, checkGameLock, (req: Request, res: Response) => {
  if (!req.sessionId) {
    res.status(401).json({
      success: false,
      error: '未认证',
  ...
  // 执行商业投资
  const result = executeCommerceInvestment(
    factionId,
    samuraiId,
    territoryId,
    Number(amount)
