 ...
 * 
 * Feature: mobile-and-system-update, Property 1: 商业投资成功率边界约束
 */
describe('Property 1: 商业投资成功率边界约束', () => {
  it('should always return success rate between 5% and 95% for any civil value and amount', () => {
    fc.assert(
      fc.property(
        fc.integer({ min: 0, max: 150 }), // 文治值范围（包括极端值）
        fc.integer({ min: COMMERCE_MIN_AMOUNT, max: 100000 }), // 投入金额范围
        (civilValue, amount) => {
          const result = calculateCommerceSuccessRate(civilValue, amount);
  ...
  it('should return 5% (minimum) for low civil value and minimum amount', () => {
    // 文治40 + 投入1000石 = 5% (下限)
    const result = calculateCommerceSuccessRate(40, COMMERCE_MIN_AMOUNT);
    expect(result).toBe(0.05);
  });

  it('should return 95% (maximum) for high civil value and high amount', () => {
    // 文治100 + 投入50000石 = 100% (上限95%)
    const result = calculateCommerceSuccessRate(100, 50000);
    expect(result).toBe(0.95);
  });

  it('should clamp to 5% for extremely low inputs', () => {
    fc.assert(
      fc.property(
        fc.integer({ min: 0, max: 30 }), // 极低文治值
        fc.integer({ min: COMMERCE_MIN_AMOUNT, max: 2000 }), // 低投入金额
        (civilValue, amount) => {
          const result = calculateCommerceSuccessRate(civilValue, amount);
          expect(result).toBeGreaterThanOrEqual(0.05);
        }
  ...
  it('should clamp to 95% for extremely high inputs', () => {
    fc.assert(
      fc.property(
        fc.integer({ min: 100, max: 200 }), // 极高文治值
        fc.integer({ min: 50000, max: 100000 }), // 高投入金额
        (civilValue, amount) => {
          const result = calculateCommerceSuccessRate(civilValue, amount);
          expect(result).toBeLessThanOrEqual(0.95);
        }
  ...
  it('should calculate approximately 50% for baseline values (civil 70, amount 25500)', () => {
    // 基准：文治70 + 投入25500石 ≈ 50%成功率
    const result = calculateCommerceSuccessRate(70, 25500);
